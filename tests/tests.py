...
class VersionTestCase(TestCase):
    def setUp(self) -> None:
        ...
        for item in data[:50000]:
            ...
            try:
                ...
                InvalidVersion
            ) as error:  # Triggers when 'parse' encounters a faulty input.
                # print(f"{error}: '{item}' is not a valid version string.")
                faulty_inputs += 1
                continue
            except (
                ValueError
            ) as error:  # Triggers when a version segment is too large to be encoded.
                overflow_errors += 1
    ...
    def test_instance_comparison(self):
        ...
        for _ in range(
            ...
            assert (version_obj1 > version_obj2) == (
                version_int1 > version_int2
            )
